...
class DeviceHistograms {
 public:
  DeviceHistograms(int gpu, size_t num_gpus, size_t num_blocks, DeviceAllocator& device_allocator,
      ...
      : gpu_(gpu), num_gpus_(num_gpus), num_blocks_(num_blocks),
        device_allocator_(device_allocator), stream_pool_(stream_pool) {
    CheckCudaError(cudaSetDevice(gpu));

    // Upstream: 7个独立的alloc+memset调用, 无法知道总共分了多少显存.
    // AJB: 先算总量, 一次性报告, 让chunk_size决策有数据依据.
    ...
    total_alloc_bytes_ += global_scatter_offsets_size;
    ...
    total_alloc_bytes_ += block_local_histograms_size;
    ...
    total_alloc_bytes_ += mgpu_histograms_size;
  ...
  // AJB: 这组histogram占了多少显存 — 诊断OOM时直接可查
  size_t total_bytes() const { return total_alloc_bytes_; }
  size_t num_blocks() const { return num_blocks_; }

  void debug_dump(const char* tag = "") const {
    fprintf(stderr, "[DEBUG][DeviceHistograms] %s GPU=%d blocks=%zu gpus=%zu total=%zuKB\n",
            tag, gpu_, num_blocks_, num_gpus_, total_alloc_bytes_ >> 10);
  }
