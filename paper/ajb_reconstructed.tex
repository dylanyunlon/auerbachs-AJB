...
% Final reconstructed LaTeX (self-contained; compiles with no external .bib).
...
%
% Claude-1 (M001-M025): Title + Abstract + Section 1 + Section 2 (Algorithm 1)
% Claude-2 (M026-M050): Section 3 Correctness and Balance Guarantees
% Claude-3 (M051-M075): Section 4 Experimental Design + Section 5 lead-in
% Claude-4 (M076-M100): Section 5.1-5.4 (RQ1-RQ4) + Figures 2-5 + Tables 1-2
% Claude-5 (M101-M125): Section 5.5-5.6 (RQ5-RQ6) + Figures 6-7
% Claude-6 (M126-M150): FINAL ASSEMBLY — neurips_2026 preamble, Section 6
%                        Related Work, Section 7 Conclusion, inline bibliography;
%                        whole-document compile with zero undefined references.
% NOTE on register: academic systems paper. Mechanisms are named by their role
% (e.g. "tier-routed transfer scheduler", "merge-path partition boundaries"),
% never by source-file names.
%
% Preamble matches the project shell (neurips_2026). A minimal neurips_2026.sty
% is vendored alongside this file so the document is self-contained; replace it
% with the official style file for camera-ready formatting if desired.
%=============================================================================
...
\State \quad Probe the interconnect to classify device pairs into a
       fast P2P tier (NVLink) and a slow tier (PCIe / host)
\State \quad Stratify the build partitions into transfer tiers by
...
\State \quad Size device-local chunks to $0.8\times$ free device memory,
       capped at $\lceil |R|/M \rceil$ per device
\Statex
\State \textbf{Runtime initialization:}
\State \quad Instantiate a tier-routed transfer scheduler with periods
       $K_x, \{K_j\}_{j=1}^{N}$ and warmup horizon $T_{\mathrm{warm}}$
\State \quad For each device $m$: replicate the initial partitioning
       $x^m_0 \gets x_0$ and structures $s^{j,m}_{-1}\gets s^j_{-1}$
\Statex
\For{$t = 0, \ldots, T-1$}
  \ForAll{devices $m = 0, \ldots, M-1$ \textbf{in parallel}}
    \State Sort / radix-partition local chunk $t$ on device $m$, then
           binary-search-join it against the co-resident probe range
    \State $\hat{g}^m_t \gets \mathrm{cap}(\Delta\text{key-count}(x^m_t),\,\rho)$
    ...
    \For{$j = 1$ \textbf{to} $N$}
      \If{$t < T_{\mathrm{warm}}$ \textbf{or} $t \bmod K_j = 0$}
        \Comment{transfer $s^j$ across tier boundary}
        \State $s^{j,m}_t \gets \textsc{Update}^j(\mathbb{E}_m[s^{j}_{t-1}],\ \hat{g}^m_t)$
      \Else
        \State $s^{j,m}_t \gets \textsc{Update}^j(s^{j,m}_{t-1},\ \hat{g}^m_t)$
               \Comment{local step}
      \EndIf
    \EndFor
    \If{$t < T_{\mathrm{warm}}$ \textbf{or} $t \bmod K_x = 0$}
      \Comment{re-balance and transfer build partitioning}
      \State $x^m_{t+1} \gets \textsc{Part}(\textsc{HostOpt}(\mathbb{E}_m[x^m_t]),\ \hat{g}^m_t,\ \eta_t,\ \{s^{j,m}_t\})$
    \Else
      \State $x^m_{t+1} \gets \textsc{Part}(x^m_t,\ \hat{g}^m_t,\ \eta_t,\ \{s^{j,m}_t\})$
    \EndIf
  \EndFor
\EndFor
\end{algorithmic}
...
%-----------------------------------------------------------------------------
\subsection{Skew-Adaptive Host Re-Balancer (RQ5)}
\label{sec:rq5}
%-----------------------------------------------------------------------------

We ablate the host re-balancer for AJB on a $700$\,M-tuple workload, comparing
plain parallel multiway-merge averaging of the boundary vector with a
skew-adaptive re-balancer that applies a momentum term (momentum $0.9$, outer
step $1.0$) biasing boundaries toward dense key ranges. The experiment uses the
A6000$\times$2 $+$ H100 server in a medium-frequency regime ($K_x = 32$, $K_u =
3K_x$, $K_v = 6K_x$), with the partitioning warm-started from a single
eager-repartition pass. While the balance bound of Theorem~\ref{thm:sgdm} does
not directly cover the momentum-biased re-balancer, the structure of the
cadence-penalty factor $\psi$ (Eq.~\ref{eq:step_size}) suggests that a higher
boundary-transfer frequency ($p_u$) should permit a larger admissible chunk size
($\eta_0 \propto 1/\sqrt{\psi}$).

As shown in Figure~\ref{fig:nesterov}, two findings emerge. First, at the medium
transfer frequency ($K_x = 32$), AJB reaches within $1\%$ of the partition
balance of eager repartitioning---a marked improvement over the infrequent
regime ($K_x = 256$). This is consistent with the quadratic dependence $\psi
\propto 1/p_x^2$: a moderate increase in transfer frequency yields a
disproportionately large improvement in the chunk-size budget and hence in
copy/compute overlap. Second, the skew-adaptive re-balancer improves balance by
${\approx}\,0.5\%$ over plain averaging on the skewed workloads, while preserving
the practical benefits of the decoupled schedule: clean admission of a newly
attached device, reduced per-chunk count noise, and prevention of
buffer-occupancy divergence on the hot device.

\begin{figure}[t]
\centering
% [Figure 6 graphic; populated when the empirical run is rendered.]
\caption{Ablation of the host re-balancer for AJB on a $700$\,M-tuple workload
in a medium-frequency regime ($K_x = 32$), showing convergence to a balanced
partition in wall-clock time (left) and in chunk groups (right). At this
transfer frequency, AJB's final balance is within $1\%$ of eager repartitioning.
The skew-adaptive re-balancer provides a further improvement over plain
averaging.}
\label{fig:nesterov}
\end{figure}

\paragraph{Takeaway.}
A moderate transfer frequency ($K_x = 32$) lets AJB approach eager-repartition
balance quality. The skew-adaptive re-balancer provides an additional balance
improvement while preserving the cross-tier efficiency and device
fault-tolerance of the decoupled schedule. We further contrast transferring the
merge-path structures against purely device-local schemes at the $1$\,B-tuple
scale in the appendix.

%-----------------------------------------------------------------------------
\subsection{Random-Order Merge-Path Enumeration as the Build Phase (RQ6)}
\label{sec:rq6}
%-----------------------------------------------------------------------------

To assess the generality of the decoupled transfer principle beyond sort-merge
and hybrid-radix build phases, we integrate AJB with a random-order merge-path
enumeration~\citep{chen2023renum}, a single-structure scheme that draws matching
ranges in randomized order to bound the cost of re-deriving balanced boundaries
after a device is added. Since this enumeration reconciles partitions through a
single randomized access structure rather than maintaining a separate
buffer-occupancy summary, the transfer schedule reduces to two periods: $K_x$
for the build partitioning and $K_u$ for the enumeration's access structure. We
set $K_u = 3K_x$ with $K_x = 32$.

As shown in Figure~\ref{fig:muon}, AJB matches the output cardinality and balance
of the uniform-period enumeration baseline across input scales from $16$\,M to
$360$\,M tuples. By decoupling the transfer frequencies, AJB moves more than
$1.5\times$ fewer bytes than the uniform-period baseline, confirming that the
residency half-life principle extends to build phases whose reconciliation
mechanism differs fundamentally from the sort-merge boundary vector.

Randomized enumeration concentrates work in the densest key ranges under heavy
fan-out, which is relevant to the composed pipeline: when the slow-tier transfer
operates on truncated boundary counts, the concentrated error could bias the
randomized access toward an unbalanced split. Full-width count accumulation in
the cross-tier transfer path (Section~\ref{sec:timescales}) mitigates this by
ensuring the access structure receives exact counts before the randomized
enumeration is applied.

\begin{figure}[t]
\centering
% [Figure 7 graphic; populated when the empirical run is rendered.]
\caption{Output-cardinality and balance comparison between the uniform-period
random-order enumeration ($K = 32$) and AJB with decoupled enumeration ($K_x =
32$, $K_u = 96$) across input scales ($16$\,M, $125$\,M, $360$\,M). AJB matches
the baseline at all scales while moving more than $\mathbf{1.5\times}$ fewer
bytes across the slow tier. Full experimental details appear in the appendix.}
\label{fig:muon}
\end{figure}

\paragraph{Takeaway.}
AJB composes with single-structure build phases that reconcile partitions through
a randomized access mechanism. The decoupled schedule reduces the access
structure's transfer frequency relative to the build partitioning, maintaining
output cardinality and balance while lowering slow-tier volume. This demonstrates
that the half-life-guided transfer principle is \emph{build-phase-agnostic}: it
applies whenever the join maintains running summaries whose drift timescales
differ.
...
In multi-GPU joins on a uniform peer-to-peer fabric, devices exchange
partitioned build and probe relations at every partitioning level, incurring
data movement linear in relation size over a bandwidth-optimal
ring~\citep{maltry2025multigpu,rui2020efficient}. When the interconnect is
heterogeneous or the relations exceed aggregate device memory, this cross-device
traffic dominates wall-clock join time~\citep{paul2021revisiting}, as devices
stall on the slowest link. Out-of-core and once-only partitioning
strategies~\citep{kim2009sortvshash,balkesen2013multicore} reduce traffic by
moving each tuple across the device boundary a single time after a host-side
partitioning pass, lowering cross-device rounds by a factor proportional to the
join's depth. Heuristic placements that keep partitions
device-local~\citep{DeWitt1992practical} or recompute boundaries from scratch
after each phase forgo robust balance guarantees under skew.

Hybrid sort-merge and radix-partitioning joins~\citep{sioulas2019hardware} are
favored for billion-tuple inputs because they overlap device-local sorting with
cross-device merging. However, their balance is sensitive to key skew, requiring
a carefully chosen partition boundary, and they maintain several
relation-resident structures whose consistency across devices governs
correctness. The state-of-the-art P2P-enabled multi-GPU
join~\citep{maltry2025multigpu} reconciles all such structures at a single
uniform cadence, which triples the cross-device payload relative to once-only
partitioning, since the transfer cost scales with the number of structures.
Random-order join enumeration~\citep{chen2023renum} bounds the cost of
re-deriving balanced boundaries when the device set changes. For further related
work, including compression of transferred partitions and skew-resilient
partitioning, see the appendix.
...
Auerbachs-AJB reconciles interconnect efficiency with exact-output correctness in
multi-GPU hash joins on heterogeneous fabrics. By assigning independent
cross-tier transfer periods to the build partitions and the merge-path
structures, and grounding those periods in each structure's residency half-life,
we obtain a provably balanced join that moves $\mathbf{170\times}$ fewer bytes
across the slow tier than eager per-level repartitioning and $\mathbf{2\times}$
fewer than the prior state-of-the-art uniform-cadence P2P join, while preserving
output cardinality and device fault-tolerance at billion-tuple scale. Our
findings yield clear guidelines: (i)~transfer the build partitions
\textbf{frequently}, and (ii)~transfer the merge-path structures \textbf{less
often}, in proportion to their half-lives. These insights open avenues for future
work, including per-range adaptive transfer frequencies, compression of the
transferred boundary vector, and emerging settings such as joins spanning
disaggregated-memory pools and multi-node accelerator fabrics. As join workloads
scale across increasingly heterogeneous hardware, we envision AJB as a template
for efficient, resilient relational processing on mixed-bandwidth platforms.

%=============================================================================
% Bibliography — inline so the document is self-contained (no external .bib /
% bibtex pass required). Keys match all \citep/\citet uses in Sections 1-7.
%=============================================================================
...
\newblock \emph{Mathematical Programming}, 199(1):165--214, 2023.

\bibitem[Balkesen et al.(2013)]{balkesen2013multicore}
C.~Balkesen, G.~Alonso, J.~Teubner, and M.~T. \"Ozsu.
\newblock Multi-core, main-memory joins: Sort vs.\ hash revisited.
\newblock \emph{Proceedings of the VLDB Endowment}, 7(1):85--96, 2013.

\bibitem[Chen et al.(2023)]{chen2023renum}
P.~Chen and collaborators.
\newblock Random-order enumeration for relational joins.
\newblock \emph{arXiv preprint}, 2023.

\bibitem[DeWitt \& Gray(1992)]{DeWitt1992practical}
D.~J. DeWitt and J.~Gray.
\newblock Parallel database systems: The future of high performance database
systems.
\newblock \emph{Communications of the ACM}, 35(6):85--98, 1992.
...
\bibitem[Kim et al.(2009)]{kim2009sortvshash}
C.~Kim, T.~Kaldewey, V.~W. Lee, E.~Sedlar, A.~D. Nguyen, N.~Satish,
J.~Chhugani, A.~Di~Blas, and P.~Dubey.
\newblock Sort vs.\ hash revisited: Fast join implementation on modern
multi-core {CPU}s.
\newblock \emph{Proceedings of the VLDB Endowment}, 2(2):1378--1389, 2009.

\bibitem[Maltry et al.(2025)]{maltry2025multigpu}
M.~Maltry and collaborators.
\newblock A four-dimensional analysis of partitioned and sort-merge joins on
multiple {GPU}s.
\newblock \emph{Proceedings of the VLDB Endowment}, 2025.

\bibitem[Paul et al.(2021)]{paul2021revisiting}
J.~Paul, S.~Lu, B.~He, and C.~T. Lau.
\newblock Revisiting hash join on graphics processors: A decade later.
\newblock In \emph{IEEE International Conference on Data Engineering}, 2021.

\bibitem[Rui et al.(2020)]{rui2020efficient}
R.~Rui, H.~Li, and Y.-C. Tu.
\newblock Efficient join algorithms for large database tables in a multi-{GPU}
environment.
\newblock \emph{Proceedings of the VLDB Endowment}, 14(4):708--720, 2020.

\bibitem[Sioulas et al.(2019)]{sioulas2019hardware}
P.~Sioulas, P.~Chrysogelos, M.~Karpathiotakis, R.~Appuswamy, and A.~Ailamaki.
\newblock Hardware-conscious hash-joins on {GPU}s.
\newblock In \emph{IEEE International Conference on Data Engineering}, pages
698--709, 2019.
