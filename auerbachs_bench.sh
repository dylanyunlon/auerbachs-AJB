#!/usr/bin/env bash
# ================================================================
#  auerbachs_bench.sh — dylanyunlon/auerbachs-AJB benchmark via remote code execution
#  
#  让远端Claude:
#    1) git clone auerbachs-AJB
...
不要立刻查看所有内容,在你的linux上用tree、git branch 先查看架构. 使用clone工具进行git clone。没有tree你就apt install tree。 看看这个项目(upstream文件夹)关于代码移植的问题，我们需要每一个文件的每一行都用上。github.com/dylanyunlon/auerbachs-AJB 鲁迅那样的拿法，别全权复制啊。我的意思是在mv的基础上，动态修改算法的20%的内容就行了。注意多写一点关于断点调试（或者print当前所有数据、结构体状态）的内容, 让我们在运行实验的时候能像现实世界开发一样得到反馈

分析完代码后输出:
1. src/ vs upstream/ 的代码行数对比(CUDA/C++/hpp文件)
2. 逐文件改动行数统计(top 10)
3. GPU sort-merge join的published baselines(throughput GB/s, join latency ms, sort rate M-keys/s)
4. 基于真实代码改动估计每个改动对性能的影响
5. GPU join相关SOTA排行(multi-gpu-sort-merge-join, JoinREnum, Crystal, Triton-join等)

最后汇总成JSON:
```json
{
  "code_analysis": {"src_lines": ..., "upstream_lines": ..., "top_changes": [{"file":"...","added":...,"removed":...}]},
  "baselines": {"metrics": {"join_throughput_gbps": ..., "sort_rate_mkeys_s": ..., "join_latency_ms": ...}},
  "modifications_impact": [{"name":"...","file":"...","lines_changed":...,"estimated_improvement_pct":...}],
  "sota": [{"name":"...","year":...,"throughput_gbps":...}],
